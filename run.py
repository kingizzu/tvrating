...
def genetic_algorithm(initial_schedule, generations=GEN, population_size=POP, crossover_rate=0.8, mutation_rate=0.2, elitism_size=EL_S):
    ...
    for generation in range(generations):
        ...
        while len(new_population) < population_size:
            ...
            # Ensure both children schedules do not have duplicate programs
...
if st.button("Run"):
    ...
    # Combine the initial best schedule with the genetic algorithm results (unique shows)
    final_schedule = list(set(initial_best_schedule).union(genetic_schedule))[:len(all_time_slots)]
